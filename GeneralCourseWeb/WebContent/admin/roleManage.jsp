<%@page import="cqut.dao.PerssionDao"%>
...
<!-- <div class="function">
    ...
    <button type="button" class="button button-primary button-small" data-toggle="modal" data-target="#myModal">增加</button>
      ...
      <form action="<%=path%>/deleteRoleServlet">               
    <table  class="table">
            <colgroup>   
                 <col style="width:25%;" />
                <col style="width:25%" />
        ...
        <thead>
          <tr> <button type="button" class="button button-primary button-small" data-toggle="modal" data-target="#myModal">增加</button>
            ...
            <tr class="active">
                ...
                <th data-field="bianji" data-width="120px">roleID</th>
                 <th data-field="bianji" data-width="120px">chooseID</th>
                <th data-field="bianji" data-width="120px">角色名称</th>
        ...
        <%for(int i = 0;i<list1.size();i++) { %>
                <tr >
                        <td>
                            <input type="checkbox" selected="selected" name=<%=i+1 %> value=<%=list1.get(i).get("roleID").toString().substring(0, list1.get(i).get("roleID").toString().length())%>/>
                        </td>
                         <td>
                              <%=list1.get(i).get("roleID")%>
                            </td>
                        <td>
                              <%=i+1%>
                            </td>
                            <td>
                                    <%=list1.get(i).get("roleName")%>
                                </td>
                        <!-- <td colspan="6">
                            <button type="button" class="button button-primary button-small" data-toggle="modal" data-target="#myModal">增加</button>
